...
MigrationBuilder.prototype = {
    ...
    columns: function ( table, columns, drop )
    {
        ...
        addColumn += this.Columns( aC, columns, ' }, callback );', true );
    ...
    changeKeys: function ( table, keys, drop, index )
    {
        var addKey = 'async.series([',
            ...
            firstLine = true,
            firstKey = true,
            index = ( drop && index !== undefined ) ? index : 0,
            kI = 0, _kI = 0;
    ...
    keys: function ( table, keys, drop, index )
    {
        var addKey = 'async.series([',
            dropKey = 'async.series([',
            firstLine = true,
            index = ( drop && index !== undefined ) ? index : 2;
    ...
    engine: function ( table, engines, drop )
    {
        ...
        var createEngine = "db.runSql('" + sql + "', '" + engines[ 0 ] + "', callback);";
        var dropEngine = '';

        if ( engines[ 1 ] )
            dropEngine = "db.runSql('" + sql + "', '" + engines[ 1 ] + "', callback);";
        else
