...
DBMigrateMigrationBuilder.prototype = {
    ...
    {
        //db.addForeignKey(tableName, referencedTableName, keyName, fieldMapping, rules, callback)
        var addFK = 'async.series([',
            firstLine = true,
            dropFK = 'async.series([',
            fk = util.format( 'db.addForeignKey.bind(db, \'%s\', \'%%s\', \'%%s\', %%s, %%s)', table ),
            dk = util.format( 'db.removeForeignKey.bind(db, \'%s\', \'%%s\')', table ),
            oKeys = Object.keys( keys );
        ...
        for( kI = 0; kI < oKeys.length; ++kI )
        {
            ...
            addFK += util.format( fk, key[ 0 ][ 2 ], oKeys[ kI ], fieldMapping, rules );
            dropFK += util.format( dk, oKeys[ kI ] );
        }
