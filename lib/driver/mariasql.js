...
MariasqlDriver.prototype = {
            ...
            self.runSql( query, true )
               .on( 'result', function( res )
               {
                  ...
                  res.on( 'row', function( row )
                  {
                     ...
                     row[1] = [ _match[1], _match[2], _match[3] ];
                     row[2] = ( row[2] === 'YES' ) ? true : false;
                     row[6] = ( row[5] === 'auto_increment' ) ? true : false;
                     row[5] = row[3];
                     row[3] = row[4];
                     row.splice( 4, 1 );
   ...
   {
      var self = this;
      var db = Array(), counter = 0, query = 'SHOW INDEX FROM %s.%s;';
      db[0] = Array();
      db[1] = Array();
      var len = ( tables['tables'][0].length > tables['tables'][1].length ) ?
                  tables['tables'][0].length : tables['tables'][1].length;
