...
Builder.prototype = {
    ...
    templating: function ( config )
    {
        ...
        inc = this.tUp( config, inc );
        inc = this.tChange( config, inc );
        inc = this.tDown( config, inc );
    ...
    /**
      * Template Creating Actions
      */ 
    tUp: function( config, inc )
    {
        var self = this,
            ...
            fForeign = internals.diff.fullforeigns,
            file = '';
        ...
        self.checkCap( 'foreign_keys', function()
        {
            for ( i = 0; i < tables[ 0 ].length; ++i )
            {
                if( fForeign[ 0 ][ tables[ 0 ][ i ] ] && fForeign[ 0 ][ tables[ 0 ][ i ] ].length > 0 )
                {
                    file = formatName( config.filestring, tables[ 0 ][ i ], 'setFKs', inc++, new Date() );
                    self.write( file, internals.template.foreign( tables[ 0 ][ i ], 
                        self.buildArr( 
                                fForeign[ 0 ][ tables[ 0 ][ i ] ]
                        ), false
                    ) );
                }
            }
    ...
    /**
      * Template addition/change actions
      */
    tChange: function( config, inc )
    {
        var self = this,
            tables = internals.diff.tables,
            columns = internals.diff.columns,
        ...
        self.checkCap( 'foreign_keys', function()
        {
            for ( kI = 0; kI < oKeys.length; ++kI )
            {
                ...
                // changing oKeys[ kI ]
                // tsp[oKeys[ kI ]]
                if ( tsp[ oKeys[ kI ] ].foreign_keys )
                {
                    if ( Object.keys( tsp[ oKeys[ kI ] ].foreign_keys[ 2 ] ).length )
                    {
                        file = formatName( config.filestring, oKeys[ kI ], 'changeFKs', inc++, new Date() );
                        /*self.write( file, internals.template.changeKeys( oKeys[ kI ], 
                            tsp[ oKeys[ kI ] ].foreign_keys[ 2 ], false, 1 ) );*/
                    }

                    if ( Object.keys( tsp[ oKeys[ kI ] ].foreign_keys[ 1 ] ).length )
                    {
                        file = formatName( config.filestring, oKeys[ kI ], 'dropFKs', inc++, new Date() );
                        //self.write( file, internals.template.keys( oKeys[ kI ], tsp[ oKeys[ kI ] ].foreign_keys[ 1 ], true, 0 ) );
                    }

                    
                    if ( Object.keys( tsp[ oKeys[ kI ] ].foreign_keys[ 0 ] ).length )
                    {
                        file = formatName( config.filestring, oKeys[ kI ], 'addFKs', inc++, new Date() );
                        //self.write( file, internals.template.keys( oKeys[ kI ], tsp[ oKeys[ kI ] ].foreign_keys[ 0 ], false, 0 ) );
                    }
                }            
            }
        } );
    ...
    /**
      * Template drop actions
      */
    tDown: function( config, inc )
    {
        var self = this,
            tables = internals.diff.tables,
            columns = internals.diff.columns,
        ...
        self.checkCap( 'foreign_keys', function()
        {
            ...
            {
                if( fForeign[ 1 ][ tables[ 1 ][ i ] ] && fForeign[ 1 ][ tables[ 1 ][ i ] ].length > 0 )
                {
                    file = formatName( config.filestring, tables[ 1 ][ i ], 'dropFKs', inc++, new Date() );
                    self.write( file, internals.template.foreign( tables[ 1 ][ i ], 
                        self.buildArr( 
                                fForeign[ 1 ][ tables[ 1 ][ i ] ]
                        ), true
                    ) );
                }
            }
        ...
        self.checkCap( 'indizies', function()
        {
            for ( i = 0; i < tables[ 1 ].length; ++i )
            {
                if( fKeys[ 1 ][ tables[ 1 ][ i ] ] && fKeys[ 1 ][ tables[ 1 ][ i ] ].length > 0 )
                {
    ...
    foreign_keys: function ( self, db )
    {
        ...
        internals.diff.fullforeigns = db;
        ...
        for ( var i = 0; i < internals.avTables.length; ++i )
        {
            ...
            if ( db[ 0 ][ internals.avTables[ i ] ] || db[ 1 ][ internals.avTables[ i ] ] )
            {
                ...
                /**
                  * Build an object to scope and organize the foreign keys.
                  */
                for ( o = 0; o < 2; ++o )
                {
                    availableKeys[ o ] = self.buildArr( availableKeys[ o ] );
                ...
                /**
                  * First checking for missing keys or added keys.
                  */
                newKeys = self.buildArr( keys[ 0 ] );
                lostKeys = self.buildArr( keys[ 1 ] );
                // end collecting
                ...
                /**
                  * Begin on layer 2, searching for changes within foreign keys.
                  */
                if ( availableKeys[ 0 ] )
                {
                    var oKeys = Object.keys( availableKeys[ 0 ] );
                    for ( kI = 0; kI < oKeys.length; ++kI )
                    {
                        ...
                        chgKeys[ oKeys[ kI ] ] = chgKeys[ oKeys[ kI ] ] || [];
                        ...
                        /**
                          * Building an object from the diff result.
                          */
                        for ( o = 0; o < 2; ++o )
                        {
                            availableKeysChg[ o ] = self.buildArr( availableKeysChg[ o ] );
                            ...
                            {
                                var oChgKeys = Object.keys( availableKeysChg[ o ] );
                                for ( var mkI = 0; mkI < oChgKeys.length; ++mkI )
                                {
                                    /**
                                      * Replace _diff in string to be able to diff
                                      */
                                    if( o === 1 )
                                    {
                                        var repRef = availableKeysChg[ o ][ oChgKeys[ mkI ] ];
                                        for( var rI = 0; rI < repRef.length; ++rI )
                                        {
                                            availableKeysChg[ o ][ oChgKeys[ mkI ] ][ rI ][ 0 ] = 
                                            availableKeysChg[ o ][ oChgKeys[ mkI ] ][ rI ][ 0 ].replace( /(.*)_diff$/, '$1' );
                                        }   
                                    }

                                    availableKeysChg[ o ][ oChgKeys[ mkI ] ] = 
                                        self.moveItem( availableKeysChg[ o ][ oChgKeys[ mkI ] ], 2, 0 );
                                }
                            }
                        }
                        ...
                        /**
                          * Finally iterating through the array, and making a complete
                          * check against the two object.
                          */
                        if ( availableKeysChg[ 0 ] )
                        {
                            var oKeysChg = Object.keys( availableKeysChg[ 0 ] );
