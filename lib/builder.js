...
Builder.prototype = {
    ...
    templating: function ( config, callback )
    {
        var self = this,
            ...
            columns = self.diff.columns,
            views = self.diff.views,
            fn = self.diff.fn,
            tsp = self.diff.tablespecific,
            //triggers = self.diff['triggers'],
            fColumns = self.diff.fullcolumns,
            fEngines = self.diff.fullengines,
            fKeys = self.diff.fullindizies,
            inc = 0, i = 0, kI = 0;
        ...
        for ( i = 0; i < tables[ 0 ].length; ++i )
        {
            ...
            self.write( file, self.template.table( tables[ 0 ][ i ], fColumns[ tables[ 0 ][ i ] ][ 0 ] ) );

            file = formatName( config.filestring, tables[ 0 ][ i ], 'setEngine', inc++, new Date() );
            self.write( file, self.template.engine( tables[ 0 ][ i ], fEngines[ 0 ][ tables[ 0 ][ i ] ] ) );

            file = formatName( config.filestring, tables[ 0 ][ i ], 'setKeys', inc++, new Date() );
            self.write( file, self.template.keys( tables[ 0 ][ i ], self.buildArr( fKeys[ 0 ][ tables[ 0 ][ i ] ] ) ) );
        }
        ...
        {
            //added columns
            if ( columns[ oKeys[ kI ] ][ 0 ][ 0 ].length > 0 )
            {
                file = formatName( config.filestring, oKeys[ kI ], 'addColumns', inc++, new Date() );

                self.write( file, self.template.columns( oKeys[ kI ], columns[ oKeys[ kI ] ][ 0 ][ 0 ] ) );
        ...
        for ( kI = 0; kI < oKeys.length; ++kI )
        {
            if ( tables[ 0 ].indexOf( oKeys[ kI ] ) !== -1 || tables[ 0 ].indexOf( oKeys[ kI ] ) !== -1 )
                return;
            ...
            if ( tsp[ oKeys[ kI ] ].indizies )
            {
                if ( Object.keys( tsp[ oKeys[ kI ] ].indizies[ 2 ] ).length )
                    self.write( file, self.template.changeKeys( oKeys[ kI ], tsp[ oKeys[ kI ] ].indizies[ 2 ] ) );
            }

            //changing engine
            if ( tsp[ oKeys[ kI ] ].engine && tsp[ oKeys[ kI ] ].engine[ 1 ] )
                self.write( file, self.template.engine( oKeys[ kI ], tsp[ oKeys[ kI ] ].engine ) );
        }
        ...
        for ( i = 0; i < tables[ 1 ].length; ++i )
        {
            ...
            self.write( file, self.template.table( tables[ 1 ][ i ], fColumns[ tables[ 1 ][ i ] ][ 1 ], true ) );

            self.write( file, self.template.engine( tables[ 1 ][ i ], fEngines[ 1 ][ tables[ 1 ][ i ] ], true ) );
            self.write( file, self.template.keys( tables[ 1 ][ i ], self.buildArr( fKeys[ 1 ][ tables[ 1 ][ i ] ] ) ) );
        }
        ...
        for ( kI = 0; kI < oKeys.length; ++kI )
        {
            ...
            if ( columns[ oKeys[ kI ] ][ 0 ][ 0 ].length > 0 )
            {
                ...
                self.write( file, self.template.columns( oKeys[ kI ], columns[ oKeys[ kI ] ][ 0 ][ 0 ], true ) );
    ...
    {
        var cmd = null;
        file = this.migrationsDir + file + '.js';

        if( !file || !text )
        {
            log.verbose( 'Error! Content of following file was empty: ' + file, text );
            return;
        }
        ...
        /**
          ...
          * You should ensure security yourself, because this could open a thread to attackers,
    ...
    columns: function ( self, db )
    {
        ...
        oKeys = Object.keys( db[ 0 ] );
        for ( kI = 0; kI < oKeys.length; ++kI )
        {
            if ( !self.diff.fullcolumns[ oKeys[ kI ] ] )
                self.diff.fullcolumns[ oKeys[ kI ] ] = Array();

            self.diff.fullcolumns[ oKeys[ kI ] ][ 0 ] = db[ 0 ][ oKeys[ kI ] ];
        }
        ...
        for ( kI = 0; kI < oKeys.length; ++kI )
        {
            ...
            self.diff.fullcolumns[ oKeys[ kI ] ][ 1 ] = db[ 1 ][ oKeys[ kI ] ];
        }

        if ( !self.diff.columns )
            self.diff.columns = Array();

        for ( i = 0; i < self.avTables.length; ++i )
        {
            var columns = Array(),
            ...
            {
                oKeys = Object.keys( available_columns[ 0 ] );
                for ( kI = 0; kI < oKeys.length; ++kI )
                {

                    if ( !available_columns[ 0 ][ oKeys[ kI ] ].identicalTo( available_columns[ 1 ][ oKeys[ kI ] ] ) )
                        chg_columns[ oKeys[ kI ] ] = available_columns[ 1 ][ oKeys[ kI ] ];
                }
            }
        ...
        for ( var i = 0; i < self.avTables.length; ++i )
        {
            ...
            if ( db[ 0 ][ self.avTables[ i ] ] || db[ 1 ][ self.avTables[ i ] ] )
            {
                ...
                for ( o = 0; o < 2; ++o )
                {
                    ...
                    if ( available_keys[ o ] )
                    {
                        oKeys = Object.keys( available_keys[ o ] );
                        for ( kI = 0; kI < oKeys.length; ++kI )
                        {
                            available_keys[ o ][ oKeys[ kI ] ] = self.moveItem( available_keys[ o ][ oKeys[ kI ] ], 2, 0 );
                        }
                    }
                }
                ...
                if ( new_keys )
                {
                    oKeys = Object.keys( new_keys );

                    for ( kI = 0; kI < oKeys.length; ++kI )
                    {
                        new_keys[ oKeys[ kI ] ] = self.moveItem( new_keys[ oKeys[ kI ] ], 2, 0 );
                    }
                }
                ...
                if ( lost_keys )
                {
                    oKeys = Object.keys( lost_keys );

                    for ( kI = 0; kI < oKeys.length; ++kI )
                    {
                        lost_keys[ oKeys[ kI ] ] = self.moveItem( lost_keys[ oKeys[ kI ] ], 2, 0 );
                    }
                }
                //end collecting
                ...
                if ( available_keys[ 0 ] )
                {
                    oKeys = Object.keys( available_keys[ 0 ] );
                    for ( kI = 0; kI < oKeys.length; ++kI )
                    {
                        ...
                        chg_keys[ oKeys[ kI ] ] = self.diffArr( available_keys[ 0 ][ oKeys[ kI ] ], available_keys[ 1 ][ oKeys[ kI ] ] );

                        available_keys_chg[ 0 ] = self.subArr( available_keys[ 0 ][ oKeys[ kI ] ], chg_keys[ oKeys[ kI ] ][ 0 ] );
                        available_keys_chg[ 1 ] = self.subArr( available_keys[ 1 ][ oKeys[ kI ] ], chg_keys[ oKeys[ kI ] ][ 1 ] );
                        ...
                        for ( o = 0; o < 2; ++o )
                        {
                            ...
                            {
                                oKeys = Object.keys( available_keys_chg[ o ] );
                                for ( kI = 0; kI < oKeys.length; ++kI )
                                {
                                    available_keys_chg[ o ][ oKeys[ kI ] ] = self.moveItem( available_keys_chg[ o ][ oKeys[ kI ] ], 2, 0 );
                                }
                            }
                        }
                        ...
                        {
                            oKeysChg = Object.keys( available_keys_chg[ 0 ] );
