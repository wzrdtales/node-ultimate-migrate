...
Builder.prototype = {
   ...
   columns: function( self, db )
   {
      ...
      for( var i = 0; i < self.avTables.length; ++i )
      {
        ...
        available_columns[0] = self.subArr( db[0][self.avTables[i]], columns[0] );
        available_columns[1] = self.subArr( db[1][self.avTables[i]], columns[1] );

        self.diff['columns'][self.avTables[i]] = columns;
      }
        ...
        if( db[0][self.avTables[i]] || db[1][self.avTables[i]] )
        {
          ...
          if( available_keys[0] )
            Object.keys(available_keys[0]).forEach(function(key) {
              ...
              chg_keys[key].push( cavailable_keys[0][key].identicalTo( available_keys[1][key] ) );
