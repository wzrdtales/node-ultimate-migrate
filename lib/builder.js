...
Builder.prototype = {
    ...
    templating: function ( config )
    {
        ...
        for ( kI = 0; kI < oKeys.length; ++kI )
        {
            ...
            if ( tsp[ oKeys[ kI ] ].indizies )
            {
                if ( Object.keys( tsp[ oKeys[ kI ] ].indizies[ 2 ] ).length )
                {
                    ...
                    self.write( file, self.template.changeKeys( oKeys[ kI ], tsp[ oKeys[ kI ] ].indizies[ 2 ], false, 1 ) );
                }
        ...
        for ( kI = 0; kI < oKeys.length; ++kI )
        {
            ...
            //dropped columns
            if ( columns[ oKeys[ kI ] ][ 0 ][ 1 ].length > 0 )
            {
                file = formatName( config.filestring, oKeys[ kI ], 'dropColumns', inc++, new Date() );
                self.write( file, self.template.columns( oKeys[ kI ], columns[ oKeys[ kI ] ][ 0 ][ 1 ], true ) );
