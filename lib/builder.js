...
Builder.prototype = {
                    ...
                    for ( kI = 0; kI < oKeys.length; ++kI )
                    {
                        ...
                        for ( o = 0; o < 2; ++o )
                        {
                            ...
                            if ( availableKeysChg[ o ] )
                            {
                                ...
                                {
                                    availableKeysChg[ o ][ oChgKeys[ kOIC ] ] = self.moveItem( availableKeysChg[ o ][ oChgKeys[ kOIC ] ], 2, 0 );
                                }
                            }
                        }
                        ...
                        if ( availableKeysChg[ 0 ] )
                        {
                            ...
                            {
                                if ( !availableKeysChg[ 0 ][ oKeysChg[ kIC ] ].identicalTo( availableKeysChg[ 1 ][ oKeysChg[ kIC ] ] ) )
                                {  
                                    if ( !chgKeys[ oKeys[ kI ] ] )
                                        chgKeys[ oKeys[ kI ] ] = Array();

                                    if ( !chgKeys[ oKeys[ kI ] ][ oKeysChg[ kIC ] ] )
                                        chgKeys[ oKeys[ kI ] ][ oKeysChg[ kIC ] ] = Array();

                                    chgKeys[ oKeys[ kI ] ][ oKeysChg[ kIC ] ][ 0 ] = availableKeysChg[ 0 ][ oKeysChg[ kIC ] ][ 0 ];
                                    chgKeys[ oKeys[ kI ] ][ oKeysChg[ kIC ] ][ 1 ] = availableKeysChg[ 1 ][ oKeysChg[ kIC ] ][ 0 ];
                                }
                            }
                        }
                    }
            ...
            if ( db[ 0 ][ internals.avTables[ i ] ] || db[ 1 ][ internals.avTables[ i ] ] )
            {
                ...
                if ( availableKeys[ 0 ] )
                {
                    ...
                    for ( kI = 0; kI < oKeys.length; ++kI )
                    {
                        ...
                        chgKeys[ oKeys[ kI ] ][ 0 ] = self.diffArr( availableKeys[ 0 ][ oKeys[ kI ] ], availableKeys[ 1 ][ oKeys[ kI ] ] );

                        availableKeysChg[ 0 ] = self.subArr( availableKeys[ 0 ][ oKeys[ kI ] ], chgKeys[ oKeys[ kI ] ][ 0 ][ 0 ] );
                        availableKeysChg[ 1 ] = self.subArr( availableKeys[ 1 ][ oKeys[ kI ] ], chgKeys[ oKeys[ kI ] ][ 0 ][ 1 ] );
                        ...
                        for ( o = 0; o < 2; ++o )
                        {
                            ...
                            if ( availableKeysChg[ o ] )
                            {
                                ...
                                {
                                    availableKeysChg[ o ][ oChgKeys[ kI ] ] = self.moveItem( availableKeysChg[ o ][ oChgKeys[ kI ] ], 2, 0 );
                                }
                            }
                        }
        ...
        for ( var i = 0; i < arr.length; ++i )
        {
            ...
            else
            {
                ...
                for ( var o = 0; o < arr[ i ].length; ++o )
                {
                    ...
                    if ( o < index )
                        moved[ i ][ o ] = arr[ i ][ o ];
                    else if ( ( o === position && !overwrite ) || o + 1 === position )
                    {
                        console.log( 'solb', o, arr[ i ][ index ], arr[ i ][ o ] );
                        moved[ i ][ o ] = arr[ i ][ index ];
                        if ( !overwrite )
                            moved[ i ][ o - 1 ] = arr[ i ][ o ];
                    }
    ...
    moveItem: function ( arr, index, position, overwrite )
    {
        ...
        {
            moved[ i ] = Array(),
            last       = arr[ i ].length - 1 === index;
            inc        = ( last ) ? ( ( overwrite ) ? 0 : 1 ) : ( overwrite || index > position ) ? 1 : 2,
            length     = ( last || overwrite ) ? arr[ i ].length - 1 : arr[ i ].length;

            console.log( last, arr[ i ].length, index, length );

            if( index > position )
            {
                move = ( overwrite && !last ) ? 1 : -1;
                dir1 = position;
                dir2 = index;
            }
            else
            ...
            for ( var o = 0; o < length; ++o )
            {
                if ( o === index )
                {
                    console.log( 'wool', o, move, arr[ i ][ o + move ] );
                    moved[ i ][ o ] = arr[ i ][ o + move ];
                }
                else if ( o === position )
                {
                    console.log( 'weil', o, arr[ i ][ index ], arr[ i ][ o ], o + ( move * -1 ) );
                    moved[ i ][ o ] = arr[ i ][ index ];
                    if ( !overwrite )
                        moved[ i ][ o + ( move * -1 ) ] = arr[ i ][ o ];
                }
                else if ( o < dir1 )
                {
                    console.log( 'ohnay', o, move, inc, arr[ i ][ o ] );
                    moved[ i ][ o ] = arr[ i ][ o ];
                }
                else if ( o < dir2 )
                {
                    console.log( 'ohway', o, inc, arr[ i ][ o ] );
                    moved[ i ][ o + 1 ] = arr[ i ][ o ];
                    console.log( moved[ i ] );
                }
                else
