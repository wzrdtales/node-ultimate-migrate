...
Builder.prototype = {
  ...
  templating: function( config, callback )
  {
    ...
    {
      if( tables[0].indexOf( key ) !== -1 || tables[0].indexOf( key ) !== -1 )
        return;
      ...
      if( tsp[key]['indizies'] )
      {
        if( Object.keys( tsp[key]['indizies'][2] ).length )
          console.log( file, self.template.changeKeys( key, tsp[key]['indizies'][2] ) );
      }

      //changing engine
      if( tsp[key]['engine'] && tsp[key]['engine'][1] )
        console.log( file, self.template.engine( key, tsp[key]['engine'] ) );
    });
        ...
        if( db[0][self.avTables[i]] || db[1][self.avTables[i]] )
        {
          ...
          if( available_keys[0] )
            Object.keys(available_keys[0]).forEach(function(key) {
              ...
              available_keys_chg[0] = self.subArr( available_keys[0][key], chg_keys[key][0] );
              available_keys_chg[1] = self.subArr( available_keys[1][key], chg_keys[key][1] );
              ...
              if( available_keys_chg[0] )
                Object.keys(available_keys_chg[0]).forEach(function(key_chg) {
                  ...
                  {
                    if( !chg_keys[key][key_chg] )
                      chg_keys[key][key_chg] = Array();

                    chg_keys[key][key_chg][0] = available_keys_chg[0][key_chg][0];
                    chg_keys[key][key_chg][1] = available_keys_chg[1][key_chg][0];
                  }
                });
            });
            ...
            Object.keys(available_keys[0]).forEach(function(key) {
              ...
              if( available_keys_chg[0] )
                Object.keys(available_keys_chg[0]).forEach(function(key_chg) {
                  ...
                  if( !available_keys_chg[0][key_chg].identicalTo( available_keys_chg[1][key_chg] ) )
                  {
                    tmp_key[key_chg][0] = available_keys_chg[0][key_chg];
                    tmp_key[key_chg][1] = available_keys_chg[1][key_chg];
